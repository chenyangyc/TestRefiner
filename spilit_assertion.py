...
from scripts.tree_sitter_query import statement_assignment_expression_query, assignment_expression_query, statement_declaration_query, statement_query, process_declaration_query, process_assignment_query, process_method_invocation_query, find_affected_variables_in_method_invocation

from data.configuration import parser
...
def find_related_variable(node, variable_related, local_variable):
    '''找到变量定义语句和赋值语句，简历变量之间的关系'''
    ...
    if node.type == 'variable_declarator':
        ...
        # a = f(b, c), b和c也应该有关系
        tmp_variable = []
        find_node_variable(value_node, tmp_variable, local_variable)
        for v1 in tmp_variable:
            for v2 in tmp_variable:
                if v1 != v2 and v1 in local_variable and v2 in local_variable:
                    variable_related[v1].append(v2)
                    variable_related[v2].append(v1)
        if name_node is not None and value_node is not None:
            name = name_node.text.decode()
            if name not in local_variable:
                print(f'Error: {name} not in local_variable')
                print(f'{node.text.decode()}')
                return
            if name not in variable_related:
                variable_related[name] = []
            variable_related[name].extend(tmp_variable)
    if node.type == 'method_invocation':
        object_node = node.child_by_field_name('object')
        object_name = ''
        ...
        # a = f(b, c), b和c也应该有关系
        tmp_variable = []
        find_node_variable(node, tmp_variable, local_variable)
        ...
        if object_name in local_variable:
            if object_name not in variable_related:
                variable_related[object_name] = []
            variable_related[object_name].extend(tmp_variable)
    ...
    if node.type == 'assignment_expression':
        ...
        if left_node is not None and right_node is not None:
            name_list = []
            find_node_variable(left_node, name_list, local_variable)
            for name in name_list:
                ...
                if name not in variable_related:
                    variable_related[name] = []
                find_node_variable(right_node, variable_related[name], local_variable)
                # variable_related[name].extend(tmp_variable)
    
    for child in node.children:
        find_related_variable(child,  variable_related, local_variable)
...
def del_assertion_information(node, code, assertion, local_variable, del_list):
    '''删掉除了选中断言之外的所有断言'''
    if node.type in ['local_variable_declaration', 'expression_statement'] and node.parent.type != 'for_statement':
        ...
        return
    for child in node.children:
        del_assertion_information(child, code, assertion, local_variable, del_list)
...
def delete_assertion_after(code, local_variable, node):
    '''删除断言之后的代码'''
    root = parser.parse(bytes(code, "utf8")).root_node
    
    assertions = []
    find_assertion(root, assertions, local_variable, [])
...
def split_assertion(code):
    ...
    find_related_variable(root, variable_related, local_variable)
    ...
    for node, variable_list in assertions:
        ...
        del_list = []
        del_extra_information(root, code, (node, new_variable), local_variable, del_list, try_node)
        del_list = sorted(del_list, key=lambda x: x[0], reverse=False)
        ...
        # 删掉assertion之后的代码
        new_code = delete_assertion_after(new_code, local_variable, node)
...
def merge_code(code1, code2):
    ...
    while i1 < l1 and i2 < l2:
        if 'assert' in code1_lines[i1]:
            while ';' not in code1_lines[i1] and i1 < l1:
                code_lines.append(code1_lines[i1])
                i1 += 1
            code_lines.append(code1_lines[i1])
            i1 += 1
        elif 'assert' in code2_lines[i2]:
            while ';' not in code2_lines[i2] and i2 < l2:
...
def merge_preorder(split_assertion_code):
    ...
    for code in split_assertion_code:
        root = parser.parse(bytes(code, "utf8")).root_node
        assertions = []
        find_assertion(root, assertions, [], [])
...
public void testCreateCategoryDataset1() {
    ...
    json_path = os.path.join(code_base, 'data', 'all_d4j_tests_filtered.json')
    ...
    print(num, num2)
    print(test_number)
    result_json_path = os.path.join(code_base, 'data', 'split_d4j_tests_filtered.json')
