...
from utils import fast_multiprocessing_without_return
import logging
import re
import json
...
# add_comment_prompt = f"Please add necessary descriptive comments to accurately reflect the test case's intention and behavior. Do not change the origin statement and structure. The test case is:\n```java\n{function_content}\n```"
...
def run_single_function(single_test_function, base_name, res_base_dir):
    proj_id = single_test_function.project_id
    bug_id = single_test_function.bug_id
    
    logger.debug(f"Start processing {base_name}")
    
    save_dir = os.path.join(res_base_dir, f'{proj_id}_{bug_id}')
    os.makedirs(save_dir, exist_ok=True)
    
    all_splited_functions = single_test_function.splitted_test_functions
    
    single_test_function.refined_test_functions = set()
    
    for cnt, single_splited_function in enumerate(all_splited_functions):
        logger.debug(f"processing splitted function: {cnt}/{len(all_splited_functions)}")
        # if 'testReplace_StrMatcher_String_int_int_int_VaryCount' not in single_splited_function.function_content:
        #     continue
        function_content = single_splited_function.function_content  
        function_content = reformat_func_name(function_content, single_splited_function.function_name)
        ...
        logger.debug(f"Invkoing the llm for comment, {cnt}th function")
        ...
        logger.debug(f"Get the response for comment, {cnt}th function")
        ...
        logger.debug(f"processed for comment, {cnt}th function")
        ...
        logger.debug(f"Invkoing the llm for rename, {cnt}th function")
        response = client.chat.completions.create(
            model="deepseek-coder",
        ...
        logger.debug(f"Get the response for rename, {cnt}th function")

        new_test = merge_rename_tests(rename_dict, add_comment_test, comment_test_ast)

        logger.debug(f"="*50 + "\n" + f"The refined case:\n {new_test} \n" + f"="*50 + "\n")
        
        single_splited_function.refine_prompt = comment_messages[0]['content'] + '\n@@\n' + rename_message[0]['content']
        single_splited_function.refine_response = add_comment_response + '\n@@\n' + rename_response
        single_splited_function.llm_refined_test_content = new_test
...
if __name__ == "__main__":
    ...
    log_file = f'{code_base}/data/logs/run_llm_refine_{date}.log'
    if os.path.exists(log_file):
        os.system(f'rm -rf {log_file}')
    
    all_cases = glob(f'{code_base}/data/d4j_base_function_2024-09-02/*/*.pkl')
    
    res_base_dir = f'{code_base}/data/d4j_llm_refine_{date}'
    if os.path.exists(res_base_dir):
        os.system(f'rm -rf {res_base_dir}')
    ...
    for i in tqdm(all_cases):
        ...
        # if 'lang' not in single_test_function.project_id.lower():
        #     continue
        tasks.append((single_test_function, base_name, res_base_dir))
        # if len(tasks) > 50:
        #     break
        # run_single_function(single_test_function, base_name, res_base_dir)

    workers = 10
    logger.debug(f"Start running with {workers} workers")
    
    processed_data = fast_multiprocessing_without_return(run_single_function, tasks, max_workers=workers)
