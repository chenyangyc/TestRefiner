...
import pickle
from data.configuration import code_base, defects4j_projects_base, extract_cov_info_tmp
...
console_handler = logging.FileHandler(f'{code_base}/data/logs/extract_evo_info.log')
...
def get_test_relevant_methods_worker(item):
    ...
    tmp_project_path = os.path.join(extract_cov_info_tmp, f"{project_id}-{bug_id}", generate_random_string(16))
    ...
    try:
        ...
        test_suite_dir = os.path.join(row_output_dir, project_id, 'evosuite/1', f'{project_id}-{str(bug_id)}f-evosuite.1.tar.bz2')
        ...
        for test_function_name in test_functions_name:
            cmd = ["defects4j", "coverage", "-w", tmp_project_path, "-t", f"{test_file}::{test_function_name}", 
                "-i", increment_path, "-s", test_suite_dir]
            logger.debug(" ".join(cmd))
            # exit()
            result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        ...
        result_queue.put({"path":os.path.join(file_dir, "data", "func_test_map_evosuite", project_id+".jsonl"), "data":item_data})
    except Exception as e:
        logger.debug(f"Error: {e}")
    finally:
        subprocess.run(['rm', '-rf', tmp_project_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)

def extract_test_coverage(d4j_info, evo_test_name_dict):
    os.makedirs(os.path.join(code_base, "data", "func_test_map_evosuite"), exist_ok=True)
    data = d4j_info
    ...
    for item in data:
        ...
        bug_id = item['bug_id']
        test_files = evo_test_name_dict[f'{proj_id}_{bug_id}']
        
        for test_file in test_files:
            tasks.append((item, test_file, code_base, result_queue))
            
        with open(os.path.join(code_base, "data", "func_test_map_evosuite", item["project_id"]+".jsonl"), "w") as f:
            f.write("")
...
if __name__ == "__main__":
    ...
    with open(f'{code_base}/data/evo_test_name_dict.pkl', 'rb') as fr:
        evo_test_name_dict = pickle.load(fr)
    ...
    extract_test_coverage(d4j_info, evo_test_name_dict)
    print("Done")
