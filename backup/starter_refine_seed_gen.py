...
from data.configuration import code_base, defects4j_projects_base, refine_seed_gen_tmp_dir
...
console_handler = logging.FileHandler(f'{code_base}/data/logs/evo_0815_refine_seed_gen_deepseek.log')
...
if __name__ == "__main__":
    date = '0730'
    api_base = "http://172.28.102.8:3336/v1"
    ...
    for cnt_index, single_function in enumerate(selected_test_functions):
        logger.debug(f'Processing {single_function.function_id}, {cnt_index} / {len(selected_test_functions)}')
        
        # single_base_function = list(single_function.refined_test_functions)[0]
        for single_base_function in list(single_function.refined_test_functions):
            proj_id = single_base_function.project_id
            bug_id = single_base_function.bug_id
            
            save_dir = os.path.join(evo_refine_seed_gen_res_dir, f'{proj_id}_{bug_id}')
    ...
    # ]
            ...
            for index, single_method in enumerate(list(methods)):
                ...
                tmp_execution_base = os.path.join(
                    refine_seed_gen_tmp_dir, date, f'{proj_id}_{bug_id}', single_base_function.function_name, str(index)
                )
                ...
                logger.debug(f"{index}-th generated test case compiled: {compiled}, passed: {passed}, syntax error: {syntax_error}")
            
                single_base_function.add_serve_as_seed_new_function(new_test_function)
            pass
        
        logger.debug(f"{'='*10} {datetime.datetime.now()} {'='*10}")
        logger.debug(f"Finished {single_function.function_id}")
        ...
        save_path = os.path.join(save_dir, single_function.function_name + '.pkl')
        logger.debug(f'Saving at {save_path}')
        with open(save_path, 'wb') as f:
            pickle.dump(single_function, f)
